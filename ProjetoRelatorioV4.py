...
def criar_documento_word(titulo, autor, resumo, itens=None, identificador=""):
...
def importar_e_processar():
  ...
  try:
    ...
    if opcao is True:
      ...
      messagebox.showinfo(
          "Sucesso!",
          f"{total} relatórios gerados com sucesso na pasta '{PASTA_DESTINO}'!",
      )

    else:
      lbl_status.config(text="Gerando relatório consolidado...")
      janela.update()
      caminho_final = criar_documento_consolidado(dados)
      messagebox.showinfo(
...
def gerar_manual():
  ...
  caminho = criar_documento_word(titulo, autor, resumo, list(itens))
  messagebox.showinfo(
...
janela = tk.Tk()
janela.title("Gerador de Relatórios Word")
janela.geometry("550x700")
janela.configure(bg="#1e1e1e")
janela.resizable(False, False)

janela.update_idletasks()
x = (janela.winfo_screenwidth() // 2) - (550 // 2)
...
tk.Label(
    janela,
    text="GERADOR DE RELATÓRIOS WORD",
    font=FONT_TITLE,
    bg=BG_DARK,
    fg="#007acc",
    pady=15,
...
entry_titulo.pack(fill="x", ipady=4, pady=(2, 8))
...
entry_autor.pack(fill="x", ipady=4, pady=(2, 8))
...
text_resumo.pack(fill="x", pady=(2, 8))
...
lista_itens.pack(fill="x", pady=(5, 10))
...
barra_progresso.pack(pady=(0, 15))
